...
/*1.2*/
SELECT FLOOR(AVG(length)/60) AS avg_hour, FLOOR(AVG(length)%60) AS avg_minute FROM film;
...
/*2.1*/
SELECT * FROM rental;
SELECT MAX(return_date) - MIN(rental_date), DATEDIFF(MAX(return_date),MIN(rental_date)) FROM rental;
/*2.2*/
SELECT *, DATE_FORMAT(rental_date, '%Y-%m') AS rental_month, DATE_FORMAT(rental_date, '%W') AS rental_weekday FROM rental LIMIT 20;
/*2.3*/
SELECT *, CASE WHEN DAYOFWEEK(rental_date) IN (1, 7) THEN 'weekend' ELSE 'workday' END AS DAY_TYPE FROM rental LIMIT 20;
...
Sort the results of the film title in ascending order. Please note that even if there are currently no null values in the rental duration column, the query should still be written to handle such cases in the future.
...
2.1 The mean film duration for each rating, and sort the results in descending order of the mean duration. Round off the average lengths 
to two decimal places. This will help identify popular movie lengths for each category.
